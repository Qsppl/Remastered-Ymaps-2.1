...
count.active = function () {
    return countImplementation ? countImplementation.active.apply(null, arguments) : true;
};
...
function normalizeRequiredNames (names) {
    ...
    ym.modules = new ModuleSystem({
        ...
        fetchBundle: function (name) {
            ...
            script.src = ym.env.hosts.static + ym.env.bundles[name];

            script.setAttribute('data-ymaps-api-ns', namespace);
            script.setAttribute('data-ymaps-api-version', '2.1.79-15597074');
            script.async = true;
        ...
        createSandbox: function (moduleSystemInstance) {
            ...
            var sandbox = ym.utils.extend({}, ym, { modules: modules });
            var dummy = {};
            Object.defineProperty(sandbox, '__mock__', {get: function () { return ym.ns['__mock__'] || dummy; }, configurable: false});
...
;
    ...
    if (ym.env.server.params.csp && !ym.supports.csp.isSupported) {
        ym.logger.warning('CSP is not supported in this browser');
    }
    ...
    function loadEngine () {
        ...
        script.src = ym.env.hosts.vectorIndex.replace('{{version}}', ym.env.vector.version);
        if (ym.env.vector.filename) {
            script.src = script.src.replace('vector.min.js', ym.env.vector.filename);
        }
