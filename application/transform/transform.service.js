...
import { callExpression, expressionStatement, functionDeclaration, identifier, importDeclaration, importSpecifier, program, stringLiteral } from "@babel/types";
...
export class TransformService {
    static transformYmapsLibrary(sourcecode) {
    ...
    static #getInitializatorModules(statement) {
        return [];
    }
    static #getInjectorModules(injector) {
        const injections = injector.expression.arguments[0].elements;
        const modules = Array();
        for (const injection of injections) {
            if (injection === null || injection.type === "SpreadElement")
                throw new Error("Invalid value of first argument of (method) ymaps.modules.define([...])! Expect FunctionDeclaration, but found " + injection?.type);
            if (!isYandexModuleInjection(injection))
                throw new TypeError("Invalid injection:" + "\n" + generate(injection));
            modules.push(...this.#getIngectionModules(injection));
        }
        return modules;
    }
    static #getIngectionModules(injection) {
        ...
        if (isPlainInjection)
            return this.#dividePlainInjectionToModules(injectionAST);
        else
            return [this.#mergeInjectionToModule(injectionAST)];
    }
    static #dividePlainInjectionToModules(injectionAST) {
        ...
        for (const state of injectionDeclaration.body.body) {
            if (isYandexModuleDefinition(state))
                definitionStatements.push(state);
            else
                commonStatements.push(state);
        }
        ...
        const nearestCommonPath = dependencyPathes.length > 1 && ("/" + findNearestCommonPath(dependencyPathes, ".").split(".").join("/"))
    ...
    static #mergeInjectionToModule(injectionAST) {
        throw new Error("Method not implemented!");
    }
}
function addModuleImportByDependencyName(module, dependency) {
...
function findNearestCommonPath(pathes, delimeter = "/") {
    if (pathes.length < 2)
        throw new Error();
    ...
    for (let level = 0; level++; true) {
        ...
        if (partsPacks.every(parts => parts[level] === levelName))
            commonParts.push(levelName);
        else
            break;
    }
    return commonParts.join(delimeter);
}
function transformYandexModuleToEsm(definition) {
    const info = readYandexModuleDefinition(definition);
    const module = program(info.declaration.body.body, undefined, "module");
    ...
    for (let specifierIdx = 0; specifierIdx < localSpecifiers.length; specifierIdx++) {
        ...
        if (localIdentifier.type !== "Identifier")
            throw new Error();
        module.body.unshift(importDeclaration([importSpecifier(localIdentifier, importedIdentifier)], stringLiteral(dependencyPath)));
    }
