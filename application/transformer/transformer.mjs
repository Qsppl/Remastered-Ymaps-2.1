    ...
    if (methods && t.isObjectExpression(methods)) {
        for (const prop of methods.properties) {
            if (t.isObjectProperty(prop)) {
                ...
                classBody.push(t.classMethod('method', prop.key, prop.value.params, methodBody));
            }
        }
    }
    // Create the constructor method
    classBody.unshift(t.classMethod('constructor', t.identifier('constructor'), [t.identifier('options')], t.blockStatement([
        t.expressionStatement(t.callExpression(t.super(), [t.identifier('options')]))
    ])));
    // Create the class declaration
    return t.classDeclaration(t.identifier(className), t.identifier(superClass), t.classBody(classBody));
}
export function transformCode(code) {
        ...
        CallExpression(path) {
            ...
            if (t.isMemberExpression(path.node.callee) &&
                ...
                if (moduleInfo) {
                    ...
                    const { imports: moduleImports, exports: moduleExports } = transformModuleDefinition(moduleInfo);
                    ...
                    if (t.isFunctionExpression(implementation)) {
                        ...
                        const defineClassCall = bodyStatements.find(stmt => t.isExpressionStatement(stmt) &&
                        ...
                        if (defineClassCall) {
                            ...
                            const classDeclaration = transformDefineClassToES6(path, moduleInfo.moduleName, superClass.name, methods);
    ...
    ast.program.body.unshift(...Array.from(imports).map(imp => createImportStatement(imp)));
